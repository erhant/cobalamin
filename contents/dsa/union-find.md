...
**Union by rank** closes that gap. `rank[x]` is an upper bound on the height of the tree rooted at `x`; always hang the shorter tree under the taller one so the combined tree stays shallow. Nothing else changes — one new field, one new line in the constructor, and a different `union`:

```typescript
// in the constructor, alongside repr
...
union(x: number, y: number): void {
  ...
  this.repr[y] = x;
  // the height only grows when both trees were equally tall
...
function minimumHammingDistance(
  ...
  // indices landing in the same component can be permuted freely
  const uf = new UnionFind(n);
  for (const [a, b] of allowedSwaps) uf.union(a, b);

  // root -> (value -> count): the multiset of source values each component holds
  ...
  // spend one copy of target[i] out of its component, or eat the mismatch
  let ans = 0;
  for (let i = 0; i < n; i++) {
    const cnt = sets.get(uf.find(i))!;
    const have = cnt.get(target[i]) ?? 0;
    if (have > 0) cnt.set(target[i], have - 1);
    else ans++;
  }
  return ans;
}
...
> [!TIP]
> [1722 Minimize Hamming Distance After Swap Operations](https://leetcode.com/problems/minimize-hamming-distance-after-swap-operations/) · [547 Number of Provinces](https://leetcode.com/problems/number-of-provinces/) · [684 Redundant Connection](https://leetcode.com/problems/redundant-connection/) · [952 Largest Component Size by Common Factor](https://leetcode.com/problems/largest-component-size-by-common-factor/) · [1631 Path With Minimum Effort](https://leetcode.com/problems/path-with-minimum-effort/) (Kruskal-style)
